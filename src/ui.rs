...
pub(crate) fn spawn_axis(
    ...
    // Center dummy entity to parent axis pieces
    commands
        .spawn((
            Transform::default(),
            GlobalTransform::default(),
        ...
        .with_children(|p| {
            p.spawn(axis(Srgba::RED.into(), 1., 0.1, 0.1)); // +X
            p.spawn(axis(Srgba::GREEN.into(), 0.1, 1., 0.1)); // +Y
            p.spawn(axis(Srgba::BLUE.into(), 0.1, 0.1, 1.)); // +Z
        });
}
